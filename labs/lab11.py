...
def insertV1(L, i):
    '''Assume L[0:i] is sorted and 0 < i < len(L).
       ...
       position so that L[0:i+1] is sorted. So insertion by swapping until
...
def search(L, i, x):
    '''Assuming L[0:i] is sorted and 0 <= i <= len(L),
       ...
       return the index of where x should go in list L'''
...
def testSearch():
    '''test function for search function, takes in list input and
    asserts what index it should be at
    '''
    # in middle
...
def insertV2(L, i):
    '''Assume L[0:i] is sorted and 0 < i < len(L).
       ...
       position so that L[0:i+1] is sorted. Another
       variation of sort where you insert the value at the index
